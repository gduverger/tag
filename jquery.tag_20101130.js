...
(function( $ ){
	...
	var KEY = { up: 38, down: 40, del: 46, tab: 9, enter: 13, escape: 27, comma: 188, page_up: 33, page_down: 34, back_space: 8 },
	...
	$.fn.extend({
		tag: function( options ) {
			...
			$(settings.editTagsSelector).click(function( event ) {
				...
				$(this).hide();
				editTags( selector, settings );
			})
			
			return this;
		},
		
		addAsTag: function( options ) {
		...
		editTag: function( selector, options ) {
			...
			return this.each(function() {
				...
				if ( $(this.inputField).length != 0 ) {
					...
					tag.inputField = $("<input type='text'>").addClass(settings.inputClass).val( tag.oldValue ).width( width );
					$(tag.inputField).keypress(function( e ) {
						var code = (e.keyCode ? e.keyCode : e.which);
						if ( code == KEY.enter ) { // Enter
							$(tag).saveTag( settings );
							createNewTag( selector, settings );
							return false;
						}
	...
	editTags = function( selector, options ) {
		var settings = $.extend( {}, DEFAULTS, options ),
			doneEditingElement;
			
		// Insets, binds, and shows "Done editing"
		if ( !doneEditingElement ) {
			doneEditingElement = $(settings.doneEditingElement).insertAfter( $(settings.editTagsSelector) ).click(function( event ) {
				if ( event.preventDefault ) { event.preventDefault(); } else { event.returnValue = false; }
				$(this).hide();
				doneEditing( selector, settings );
			});
			// bindDoneEditing( selector, settings );
		} else {
	...
	doneEditing = function( selector, options ) {
		var settings = $.extend( {}, DEFAULTS, options );
		
		// Switches to view mode
		$(selector).children().each(function() {
			if ( this.inputField ) { $(this).saveTag( settings ); }
			$(this.removeLink).hide();
			$(this).removeClass( settings.editModeClass ); // EDIT MODE
		})
	...
	createNewTag = function( selector, options ) {
		var tag, settings = $.extend( {}, DEFAULTS, options );
		
		// Create a tag element
		tag = $(settings.tagTemplate).addAsTag( settings );
